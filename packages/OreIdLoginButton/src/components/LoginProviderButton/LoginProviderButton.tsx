...
export const LoginProviderButton: FunctionComponent<LoginProviderButtonProps> =
	(props) => {
		...
		const providerConfig = useMemo(() => {
			...
			if (!metadata) {
				throw Error(
					`${provider} is not one of the supported providers. Use one of the following: ${providers
						.map((p) => p.id)
						.join(", ")}`
				);
			}
			const buttonStyle = {
				...defaultButtonStyle,
				...(metadata.style.buttonStyle || {}),
				...(props.buttonStyle || {}),
			};
			const logoStyle = {
				...defaultLogoStyle,
				...(metadata.style.logoStyle || {}),
				...(props.logoStyle || {}),
			};
		...
		const dataAttributes = Object.keys(props).reduce(
			(prev, curr) =>
				curr.startsWith("data-") ? { ...prev, [curr]: props[curr] } : prev,
			{}
		);
		return (
			<div className={props.className}>
				...
				>
					<img
						style={providerConfig.logoStyle}
						src={providerConfig.metadata.logo}
						alt={providerConfig.text}
					/>
					<span className={props.textClassName}>{providerConfig.text}</span>
				</button>
			</div>
		);
	};
